...
namespace WebCheckProject
{
    ...
    public partial class ZHHeader_BGD_BJ_HX1 : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    {
                        connection.Open();
                        query = "SELECT distinct gcmc,bgrq FROM Report_list_hx1 WHERE CNO= @NO";//根据任务单编号从对应委托单里搜索数据
        ...
        public string Getdata()
        {
            ...
            string connectionString = "Data Source=localhost;Initial Catalog=test;Integrated Security=SSPI;";
            string query = "SELECT YWno ,SNno ,qywz ,sylx ,qysd ,[wlxzqw℃] ,[wlxzsw℃] ,wlxzys ,wlxzqw , wlxztmd ,hxxzpHz ,hxxzylCO2 , hxxzqsCO2 , [hxxzCa2+] , [hxxzMg2+] , [hxxzK+ Na+] ,[hxxzCl-] , [hxxzSO42-] ,[hxxzHCO3-] ,[hxxzCO32-] ,hxxzlzzl ,hxxzzjd ,hxxzzyd ,hxxzzsyd ,hxxzyjyd ,shxlx FROM Report_list_hx1 WHERE  cno= @NO order by YWno,SNno;";
        ...
        private void ReceiveJsonData()//将数据用datatable批量插入数据库，保证数据提交后不会因为单条数据通过验证而写进数据库
        {
            using (StreamReader reader = new StreamReader(Request.InputStream))
            {
                ...
                dataTable.Columns.Add("hxxzCO32-", typeof(string));
                ...
                string deleteQuery = "DELETE FROM Report_list_hx1 WHERE cno= @NO";//从对应委托单里搜索项目编号
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
