...
namespace WebCheckProject
{
    ...
    public partial class ZHHeader_BGD_BJ_HX2 : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    {
                        connection.Open();
                        query = "SELECT distinct gcmc,bgrq FROM Report_list_hx2 WHERE CNO= @NO";//根据任务单编号从对应委托单里搜索数据
        ...
        public string Getdata()
        {
            ...
            string connectionString = "Data Source=localhost;Initial Catalog=test;Integrated Security=SSPI;"; 
            string query = "SELECT YWno ,SNno ,qywz , qysd , PHz ,[Ca2+] , [Mg2+] ,[Cl-] ,[SO42-] , [HCO3-] ,[CO32-] FROM Report_list_hx2 WHERE  cno= @NO order by YWno,SNno;";
        ...
        private void ReceiveJsonData()//将数据用datatable批量插入数据库，保证数据提交后不会因为单条数据通过验证而写进数据库
        {
            using (StreamReader reader = new StreamReader(Request.InputStream))
            {
                ...
                dataTable.Columns.Add("PHz", typeof(string));
                dataTable.Columns.Add("Ca2+", typeof(string));
                dataTable.Columns.Add("Mg2+", typeof(string));
                dataTable.Columns.Add("Cl-", typeof(string));
                dataTable.Columns.Add("SO42-", typeof(string));
                dataTable.Columns.Add("HCO3-", typeof(string));
                dataTable.Columns.Add("CO32-", typeof(string));
                ...
                string deleteQuery = "DELETE FROM Report_list_hx2 WHERE cno= @NO";//从对应委托单里搜索项目编号
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        {
                            bulkCopy.DestinationTableName = "Report_list_hx2";
                            try
                            {
