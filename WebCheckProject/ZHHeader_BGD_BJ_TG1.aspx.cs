...
namespace WebCheckProject
{
    ...
    public partial class ZHHeader_BGD_BJ_TG1 : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        connection.Open();
                        //表名修正
                        query = "SELECT distinct gcmc,bgrq FROM report_list_tg01 WHERE CNO= @NO";//根据任务单编号从对应委托单里搜索数据
        ...
        public string Getdata()
        {
            ...
            string connectionString = "Data Source=localhost;Initial Catalog=test;Integrated Security=SSPI;";
            //重写sql，字段设计一定不能有空格，更不能带标点！！！排查别人bug......
            string query = "SELECT YWno,SNno qysd, tdtrztbz, tdtrzthsl,tdtrztsmd, tdtrztgmd,tdtrztkxb ,tdtrztkxl, tdtrztbhd,yx, sx, sxzs, yxzs, yxxcsd,sxzsxcsd , [ysxs av0.5-1] ,[ysml Es0.5-1] ,[ysxs av1-2],[ysml Es1-2],[ysxs av2-4],[ysml Es2-4],[ysxs av4-8],[ysml Es4-8],[ysxs av8-16],[ysml Es8-16],htbtylsxxs, htsxqsyl,htbhzzyl,htzzsxxs, czstxs,spstxs FROM report_list_tg01 WHERE  cno= @NO order by YWno,SNno;";
        ...
        private void ReceiveJsonData()//将数据用datatable批量插入数据库，保证数据提交后不会因为单条数据通过验证而写进数据库
        {
            using (StreamReader reader = new StreamReader(Request.InputStream))
            {
                ...
                dataTable.Columns.Add("tdtrztbz", typeof(string));
                dataTable.Columns.Add("tdtrzthsl", typeof(string));
                dataTable.Columns.Add("tdtrztsmd", typeof(string));
                dataTable.Columns.Add("tdtrztgmd", typeof(string));
                dataTable.Columns.Add("tdtrztkxb", typeof(string));
                dataTable.Columns.Add("tdtrztkxl", typeof(string));
                dataTable.Columns.Add("tdtrztbhd", typeof(string));
                dataTable.Columns.Add("yx", typeof(string));
                dataTable.Columns.Add("sx", typeof(string));
                dataTable.Columns.Add("sxzs", typeof(string));
                dataTable.Columns.Add("yxzs", typeof(string));
                dataTable.Columns.Add("yxxcsd", typeof(string));
                dataTable.Columns.Add("sxzsxcsd", typeof(string));
                dataTable.Columns.Add("ysxs av0.5-1", typeof(string));
                dataTable.Columns.Add("ysml Es0.5-1", typeof(string));
                dataTable.Columns.Add("ysxs av1-2", typeof(string));
                dataTable.Columns.Add("ysml Es1-2", typeof(string));
                dataTable.Columns.Add("ysxs av2-4", typeof(string));
                dataTable.Columns.Add("ysml Es2-4", typeof(string));
                dataTable.Columns.Add("ysxs av4-8", typeof(string));
                dataTable.Columns.Add("ysml Es4-8", typeof(string));
                dataTable.Columns.Add("ysxs av8-16", typeof(string));
                dataTable.Columns.Add("ysml Es8-16", typeof(string));
                dataTable.Columns.Add("htbtylsxxs", typeof(string));
                dataTable.Columns.Add("htsxqsyl", typeof(string));
                dataTable.Columns.Add("htbhzzyl", typeof(string));
                dataTable.Columns.Add("htzzsxxs", typeof(string));
                dataTable.Columns.Add("czstxs", typeof(string));
                dataTable.Columns.Add("spstxs", typeof(string));
                ...
                //表名修正
                string deleteQuery = "DELETE FROM report_list_tg01 WHERE cno= @NO";//从对应委托单里搜索项目编号
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        {
                            //report_list_tg01
                            bulkCopy.DestinationTableName = "report_list_tg01";
                            try
                            {
