...
namespace WebCheckProject
{
    ...
    public partial class ZHHeader_BGD_BJ_TG11 : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        connection.Open();
                        //修正表名
                        query = "SELECT distinct gcmc,bgrq FROM report_list_tg12 WHERE CNO= @NO";//根据任务单编号从对应委托单里搜索数据
        ...
        public string Getdata()
        {
            ...
            string connectionString = "Data Source=localhost;Initial Catalog=test;Integrated Security=SSPI;";
            //修正表名
            string query = "SELECT YWno,SNno ,bgmd , djmd, gzztkxl,xsl , hnl ,nkhl ,ymhl , qwzhl,yryhl,yjzhl , lhwhlsyzhl, xdms, pjlj, jgx  FROM report_list_tg12 WHERE  cno= @NO order by YWno,SNno;"; 
        ...
        private void ReceiveJsonData()//将数据用datatable批量插入数据库，保证数据提交后不会因为单条数据通过验证而写进数据库
        {
            using (StreamReader reader = new StreamReader(Request.InputStream))
            {
                ...
                dataTable.Columns.Add("gzztkxl", typeof(string));
                dataTable.Columns.Add("xsl", typeof(string));
                dataTable.Columns.Add("hnl", typeof(string));
                dataTable.Columns.Add("nkhl", typeof(string));
                dataTable.Columns.Add("ymhl", typeof(string));
                dataTable.Columns.Add("qwzhl", typeof(string));
                dataTable.Columns.Add("yryhl", typeof(string));
                dataTable.Columns.Add("yjzhl", typeof(string));
                dataTable.Columns.Add("lhwhlsyzhl", typeof(string));
                dataTable.Columns.Add("xdms", typeof(string));
                dataTable.Columns.Add("pjlj", typeof(string));
                ...
                //修正表名
                string deleteQuery = "DELETE FROM report_list_tg12 WHERE cno= @NO";//从对应委托单里搜索项目编号
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        {
                            //修正表名
                            bulkCopy.DestinationTableName = "report_list_tg12";
                            try
                            {
