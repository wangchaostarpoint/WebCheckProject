...
namespace WebCheckProject
{
    ...
    public partial class ZHHeader_BGD_BJ_TG2 : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        connection.Open();
                        //表名修正
                        query = "SELECT distinct gcmc,bgrq FROM report_list_tg02 WHERE CNO= @NO";//根据任务单编号从对应委托单里搜索数据
        ...
        public string Getdata()
        {
            ...
            string connectionString = "Data Source=localhost;Initial Catalog=test;Integrated Security=SSPI;";
            string query = "SELECT YWno,SNno,lllj, sllj , fllj ,zllj , yxlj ,xzlj ,bjyxs , qlxs ,tldh ,tagfdm FROM report_list_tg02 WHERE  cno= @NO order by YWno,SNno;"; 
        ...
        private void ReceiveJsonData()//将数据用datatable批量插入数据库，保证数据提交后不会因为单条数据通过验证而写进数据库
        {
            using (StreamReader reader = new StreamReader(Request.InputStream))
            {
                ...
                dataTable.Columns.Add("lllj", typeof(string));
                dataTable.Columns.Add("sllj", typeof(string));
                dataTable.Columns.Add("fllj", typeof(string));
                dataTable.Columns.Add("zllj", typeof(string));
                ...
                dataTable.Columns.Add("tagfdm", typeof(string));
                ...
                //表名修正
                string deleteQuery = "DELETE FROM report_list_tg02 WHERE cno= @NO";//从对应委托单里搜索项目编号
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        {
                            bulkCopy.DestinationTableName = "report_list_tg02";
                            try
                            {
