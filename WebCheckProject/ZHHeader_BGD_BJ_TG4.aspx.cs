...
namespace WebCheckProject
{
    ...
    public partial class ZHHeader_BGD_BJ_TG4 : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        connection.Open();
                        //表名更正
                        query = "SELECT distinct gcmc,bgrq FROM report_list_tg04 WHERE CNO= @NO";//根据任务单编号从对应委托单里搜索数据
        ...
        public string Getdata()
        {
            ...
            string connectionString = "Data Source=localhost;Initial Catalog=test;Integrated Security=SSPI;";
            //表名更正
            string query = "SELECT YWno,SNno ,qysd , bzjszdgmd , bzjszyhsl ,yjzhl , yryhl ,sjd FROM report_list_tg04 WHERE  cno= @NO order by YWno,SNno;"; using (SqlConnection con = new SqlConnection(connectionString))
            {
        ...
        private void ReceiveJsonData()//将数据用datatable批量插入数据库，保证数据提交后不会因为单条数据通过验证而写进数据库
        {
            using (StreamReader reader = new StreamReader(Request.InputStream))
            {
                ...
                dataTable.Columns.Add("bzjszdgmd", typeof(string));
                dataTable.Columns.Add("bzjszyhsl", typeof(string));
                dataTable.Columns.Add("yjzhl", typeof(string));
                dataTable.Columns.Add("yryhl", typeof(string));
                dataTable.Columns.Add("sjd", typeof(string));
                ...
                //表名更正
                string deleteQuery = "DELETE FROM report_list_tg04 WHERE cno= @NO";//从对应委托单里搜索项目编号
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        {
                            bulkCopy.DestinationTableName = "report_list_tg04";
                            try
                            {
