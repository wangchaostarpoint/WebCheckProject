...
namespace WebCheckProject
{
    ...
    public partial class ZHHeader_BGD_BJ_TG5 : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        connection.Open();
                        //修正表名
                        query = "SELECT distinct gcmc,bgrq FROM report_list_tg05 WHERE CNO= @NO";//根据任务单编号从对应委托单里搜索数据
        ...
        public string Getdata()
        {
            ...
            string connectionString = "Data Source=localhost;Initial Catalog=test;Integrated Security=SSPI;";
            //重写sql，字段设计一定不能有空格，更不能带标点！！！
            string query = "SELECT YWno,SNno ,qysd , zygmd, zyhsl,[ysxs av0.5-1] , [ysml Es0.5-1] ,[ysxs av1-2] ,[ysml Es1-2] , [ysxs av2-4] ,[ysml Es2-4] ,[ysxs av4-8] , [ysml Es4-8] , [ysxs av8-12] , [ysml Es8-12] , [ysxs av12-16] ,[ysml Es12-16] , czstxs ,spstxs FROM report_list_tg05 WHERE  cno= @NO order by YWno,SNno;"; 
        ...
        private void ReceiveJsonData()//将数据用datatable批量插入数据库，保证数据提交后不会因为单条数据通过验证而写进数据库
        {
            using (StreamReader reader = new StreamReader(Request.InputStream))
            {
                ...
                dataTable.Columns.Add("zygmd", typeof(string));
                dataTable.Columns.Add("zyhsl", typeof(string));
                ...
                dataTable.Columns.Add("ysxs av8-12", typeof(string));
                dataTable.Columns.Add("ysml Es8-12", typeof(string));
                dataTable.Columns.Add("ysxs av12-16", typeof(string));
                dataTable.Columns.Add("ysml Es12-16", typeof(string));
                ...
                //表名修正
                string deleteQuery = "DELETE FROM report_list_tg05 WHERE cno= @NO";//从对应委托单里搜索项目编号
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        {
                            //修正表名
                            bulkCopy.DestinationTableName = "report_list_tg05";
                            try
                            {
