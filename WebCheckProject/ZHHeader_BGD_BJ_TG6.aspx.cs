...
namespace WebCheckProject
{
    ...
    public partial class ZHHeader_BGD_BJ_TG6 : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        connection.Open();
                        //修正表名
                        query = "SELECT distinct gcmc,bgrq FROM report_list_tg06 WHERE CNO= @NO";//根据任务单编号从对应委托单里搜索数据
        ...
        public string Getdata()
        {
            ...
            string connectionString = "Data Source=localhost;Initial Catalog=test;Integrated Security=SSPI;";
            //重写sql.......
            string query = "SELECT YWno,SNno ,zygmd , zyhsl, [bhkjczyljyl 100kPa] ,[bhkjczyljyl 200kPa] ,[bhkjczyljyl 300kPa],[bhkjczyljyl 400kPa] ,bhkjnjl , bhkjnmcl,[bhmjczyljyl 100kPa],[bhmjczyljyl 200kPa] ,[bhmjczyljyl 300kPa] , [bhmjczyljyl 400kPa] , bhmjnjl , bhmjnmcj FROM report_list_tg06 WHERE  cno= @NO order by YWno,SNno;"; 
        ...
        private void ReceiveJsonData()//将数据用datatable批量插入数据库，保证数据提交后不会因为单条数据通过验证而写进数据库
        {
            using (StreamReader reader = new StreamReader(Request.InputStream))
            {
                ...
                dataTable.Columns.Add("bhmjczyljyl 100kPa", typeof(string));
                dataTable.Columns.Add("bhmjczyljyl 200kPa", typeof(string));
                dataTable.Columns.Add("bhmjczyljyl 300kPa", typeof(string));
                dataTable.Columns.Add("bhmjczyljyl 400kPa", typeof(string));
                dataTable.Columns.Add("bhmjnjl", typeof(string));
                dataTable.Columns.Add("bhmjnmcj", typeof(string));
                ...
                //修正表名
                string deleteQuery = "DELETE FROM report_list_tg06 WHERE cno= @NO";//从对应委托单里搜索项目编号
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        {
                            //修正表名
                            bulkCopy.DestinationTableName = "report_list_tg06";
                            try
                            {
