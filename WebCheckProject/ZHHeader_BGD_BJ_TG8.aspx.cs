...
namespace WebCheckProject
{
    ...
    public partial class ZHHeader_BGD_BJ_TG8 : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        connection.Open();
                        //修正表名
                        query = "SELECT distinct gcmc,bgrq FROM report_list_tg08 WHERE CNO= @NO";//根据任务单编号从对应委托单里搜索数据
        ...
        public string Getdata()
        {
            ...
            string connectionString = "Data Source=localhost;Initial Catalog=test;Integrated Security=SSPI;";
            //重写sql......
            string query = "SELECT YWno,SNno ,[tdljjl ＞200%] , [tdljjl 200-60%] , [tdljll 60-40%] ,[tdljll 40-20%] ,[tdljll 20-10%] ,[tdljll 10-5%] ,[tdljll 5-2%] , [tdljsl 2-1%],[tdljsl 1-0.5%] ,[tdljsl 0.5-0.25%] , [tdljsl 0.25-0.075%] , [tdljxl <0.075%] , yxlj , xzlj ,bjyxs , qlxs,tldh ,tdmc,hnl,bz,zdgmd, zxgmd,sszrxzj,sxzrxzj, zygmd,stxs,ljpj,stph FROM report_list_tg08 WHERE  cno= @NO order by YWno,SNno;"; 
        ...
        private void ReceiveJsonData()//将数据用datatable批量插入数据库，保证数据提交后不会因为单条数据通过验证而写进数据库
        {
            using (StreamReader reader = new StreamReader(Request.InputStream))
            {
                ...
                dataTable.Columns.Add("tdljxl <0.075%", typeof(string));
                ...
                dataTable.Columns.Add("zdgmd", typeof(string));
                dataTable.Columns.Add("zxgmd", typeof(string));
                dataTable.Columns.Add("sszrxzj", typeof(string));
                dataTable.Columns.Add("sxzrxzj", typeof(string));
                dataTable.Columns.Add("zygmd", typeof(string));
                dataTable.Columns.Add("stxs", typeof(string));
                dataTable.Columns.Add("ljpj", typeof(string));
                dataTable.Columns.Add("stph", typeof(string));
                ...
                //修正表名
                string deleteQuery = "DELETE FROM report_list_tg08 WHERE cno= @NO";//从对应委托单里搜索项目编号
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        {
                            bulkCopy.DestinationTableName = "report_list_tg08";
                            try
                            {
