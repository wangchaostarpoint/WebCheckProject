...
namespace WebCheckProject
{
    ...
    public partial class ZHHeader_BGD_BJ_YS2 : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        connection.Open();
                        //修正表名
                        query = "SELECT distinct gcmc,bgrq FROM report_list_ys2 WHERE CNO= @NO";//根据任务单编号从对应委托单里搜索数据
        ...
        public string Getdata()
        {
            ...
            string connectionString = "Data Source=localhost;Initial Catalog=test;Integrated Security=SSPI;";
            //重写sql......
            string query = "SELECT YWno,SNno ,ysmc , klmd , gmd ,bhmd , xsl ,bhxsl ,bsxs , kxl ,ylyclgx ,gzdzkyqd , bhdzkyqd , rhxs , [drhbhdzkyqd] , drxs ,drzlssl , [lhwhlsyzhl]  FROM report_list_ys2 WHERE  cno= @NO order by YWno,SNno;";
        ...
        private void ReceiveJsonData()//将数据用datatable批量插入数据库，保证数据提交后不会因为单条数据通过验证而写进数据库
        {
            using (StreamReader reader = new StreamReader(Request.InputStream))
            {
                ...
                dataTable.Columns.Add("ylyclgx", typeof(string));
                ...
                dataTable.Columns.Add("drhbhdzkyqd", typeof(string));
                dataTable.Columns.Add("drxs", typeof(string));
                dataTable.Columns.Add("drzlssl", typeof(string));
                ...
                //修正表名
                string deleteQuery = "DELETE FROM report_list_ys2 WHERE cno= @NO";//从对应委托单里搜索项目编号
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        {
                            //修正表名
                            bulkCopy.DestinationTableName = "report_list_ys2";
                            try
                            {
