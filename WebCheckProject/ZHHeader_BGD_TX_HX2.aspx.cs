...
namespace WebCheckProject
{
    ...
    public partial class ZHHeader_BGD_TX_HX2 : System.Web.UI.Page
    {
        //方法重构
        ...
        private void ReceiveJsonData()//将数据用datatable批量插入数据库，保证数据提交后不会因为单条数据通过验证而写进数据库
        {
            using (StreamReader reader = new StreamReader(Request.InputStream))
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    {
                        string yw = row[0]; // 室内编号在数据数组第一个位置
                        string sn = row[1]; // 野外编号在第二个位置

                        string checkSql = @"SELECT COUNT(*) FROM Report_list_hx2 WHERE YWno = @YWno AND SNno = @SNno";
