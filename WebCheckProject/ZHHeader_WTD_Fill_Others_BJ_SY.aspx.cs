...
namespace WebCheckProject
{
    ...
    public partial class ZHHeader_WTD_Fill_Others_BJ_SY : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    {
                        connection.Open();
                        query = "SELECT distinct gcmc,Wtdw,syr,lxdh,Qyrq,Syrq,Qyr FROM commission_sheet_sy2 WHERE cno= @NO";//根据任务单编号从对应委托单里搜索数据
        ...
        public string getdata()
        {
            ...
            string connectionString = "Data Source=localhost;Initial Catalog=test;Integrated Security=SSPI;";
            string query = "SELECT Snno,Ywno,Qsdz,Sylx,Qssd,Fxmd,Ykna,Yca,Ymg,Ycl,Yso,Yhco,Yco,Zyd,Zsyd,Yjyd,Zjd,Krx,Xfw,Gxw,Zrcz,Ylco,Qsylco,Phz FROM commission_sheet_sy2 WHERE  cno = @NO order by SNno;";
        ...
        private void ReceiveJsonData()//将数据用datatable批量插入数据库，保证数据提交后不会因为单条数据通过验证而写进数据库
        {
            using (StreamReader reader = new StreamReader(Request.InputStream))
            {
                ...
                dataTable.Columns.Add("Qsdz", typeof(string));
                dataTable.Columns.Add("Sylx", typeof(string));
                dataTable.Columns.Add("Qssd", typeof(string));
                dataTable.Columns.Add("Fxmd", typeof(string));
                dataTable.Columns.Add("Ykna", typeof(string));
                dataTable.Columns.Add("Yca", typeof(bool));
                dataTable.Columns.Add("Ymg", typeof(bool));
                dataTable.Columns.Add("Ycl", typeof(bool));
                dataTable.Columns.Add("Yso", typeof(bool));
                dataTable.Columns.Add("Yhco", typeof(bool));
                dataTable.Columns.Add("Yco", typeof(bool));
                dataTable.Columns.Add("Zyd", typeof(bool));
                dataTable.Columns.Add("Zsyd", typeof(bool));
                dataTable.Columns.Add("Yjyd", typeof(bool));
                dataTable.Columns.Add("Zjd", typeof(bool));
                dataTable.Columns.Add("Krx", typeof(bool));
                dataTable.Columns.Add("Xfw", typeof(bool));
                dataTable.Columns.Add("Gxw", typeof(bool));
                dataTable.Columns.Add("Zrcz", typeof(bool));
                dataTable.Columns.Add("Ylco", typeof(bool));
                dataTable.Columns.Add("Qsylco", typeof(bool));
                dataTable.Columns.Add("Phz", typeof(bool));
                ...
                string deleteQuery = "DELETE FROM commission_sheet_sy2 WHERE cno= @NO";//从对应委托单里搜索项目编号
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        {
                            bulkCopy.DestinationTableName = "commission_sheet_sy2";
                            try
                            {
