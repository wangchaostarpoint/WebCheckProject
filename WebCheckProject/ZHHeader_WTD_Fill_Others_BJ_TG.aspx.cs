...
namespace WebCheckProject
{
    ...
    public partial class ZHHeader_WTD_Fill_Others_BJ_TG : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    {
                        connection.Open();
                        query = "SELECT distinct gcmc,Wtdw,syr,lxdh,Qyrq,Syrq,Qyr FROM commission_sheet_tg2 WHERE cno= @NO";//根据任务单编号从对应委托单里搜索数据
        ...
        public string getdata()
        {
            ...
            string connectionString = "Data Source=localhost;Initial Catalog=test;Integrated Security=SSPI;";
            string query = "SELECT Snno,Ywno,Yx,Ypzl,Ypsl,Qtsd,Bz,Hsl,Srl,Grl,Sxzs,Klfx,Yryhl,Yjzhl,Sjd,Tdfsx,Kjqdkj,Kjqdbhkj,Kjqdgjkj,Kjqdbhgjkj,Yssynxt,Yssysxdxf,Yssysxsxf,Stsp,Stcz,Jssy,Ktjlztms,Ktjlktr FROM commission_sheet_tg2 WHERE  cno= @NO order by SNno;";
        ...
        private void ReceiveJsonData()//将数据用datatable批量插入数据库，保证数据提交后不会因为单条数据通过验证而写进数据库
        {
            using (StreamReader reader = new StreamReader(Request.InputStream))
            {
                ...
                string deleteQuery = "DELETE FROM commission_sheet_tg2 WHERE cno= @NO";//从对应委托单里搜索项目编号
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
